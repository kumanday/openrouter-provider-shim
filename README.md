...
Local npx-runnable shim that enforces OpenRouter provider routing (e.g., Fireworks-only) while supporting Claude Code, OpenCode, OpenHands, and Droid.

## Why this shim exists

Many AI agent harnesses can point at an OpenAI-compatible base URL, but they cannot attach OpenRouter's per-request `provider` routing object. OpenRouter supports a `provider` object for routing preferences including `only`, `order`, `ignore`, `sort` (price, throughput, latency), performance thresholds, and max price. This shim injects these fields server-side, so end users do not need OpenRouter account-wide settings.
